...
{		
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,("%s() : default channel to %d \n",__FUNCTION__,pAd->CommonCfg.Channel));
	...
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	for (apidx=0; apidx<pAd->ApCfg.BssidNum; apidx++)
	{
		...
		sprintf(msg+strlen(msg),"bytesTx = %lld\n", pMbss->TransmittedByteCount.QuadPart);
		sprintf(msg+strlen(msg),"bytesRx = %lld\n", pMbss->ReceivedByteCount.QuadPart);
...
INT32 ShowPseInfo(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("P2 RQ2(TxS):\n"));
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("MCU CTX_Q2 frame information\n"));
