...
/*
	...
	usage: iwpriv ra0 set ITxBfDivCal=ff[:mm]
		where ff is the calFunction:
		...
		where mm is the calMethod:
			0=>default based on channel
			1,2,3,4=>Method 1,2,3,4
*/
INT	Set_ITxBfDivCal_Proc(
	IN	PRTMP_ADAPTER	pAd, 
    IN  PSTRING         arg)
{
	int calFunction, calMethod=0;
	PSTRING token;

	calFunction = simple_strtol(arg, 0, 10);
	token = strchr(arg, ':');
	if (token != NULL)
		calMethod = simple_strtol(token+1, 0, 10);

	return ITxBFDividerCalibration(pAd, calFunction, calMethod, NULL);
}
    ...
    IN  PSTRING         arg)
{
	...
	DBGPRINT((calFunction==0? RT_DEBUG_OFF: RT_DEBUG_WARN),
				("ITxBfCal Result(hex) = [0x%02x 0x%02x]\n", calParams[0], calParams[1]));

	/* Transform the result to angles and display it */
	calAngles[0] = (calParams[0]*180) / 128;
	calAngles[1] = (calParams[1]*180) / 128;
	...
	DBGPRINT((calFunction==0? RT_DEBUG_OFF: RT_DEBUG_WARN),
				("ITxBfCal Result in ATE = [%d %d](degree)\n", calAngles[0], calAngles[1]));
#endif /* RALINK_ATE */
